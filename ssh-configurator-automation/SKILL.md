...
Для обычного пользовательского запуска не стартовать `ssh-configurator` из sandboxed shell Codex. Такой запуск может открыть Electron-окно с белым экраном без элементов. Обычный frontend поднимать только как внешний GUI-процесс:

```bash
/usr/lib/ssh-configurator/ssh-configurator
```

Если запуск нужен именно из агентной сессии, выполнять его вне sandbox c escalated permissions, а не как обычную sandbox-команду.
...
- Обычный пользовательский запуск `ssh-configurator` не делать из sandbox. Белый экран без элементов считать типичным симптомом sandboxed GUI-launch; в таком случае закрыть экземпляр и перезапустить `/usr/lib/ssh-configurator/ssh-configurator` вне sandbox.

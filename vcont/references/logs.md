...
- `Size of send data is <N>` - объем данных, которые statistics/sync готовит к отправке; это та самая надпись про объем пересланных/отправляемых данных.
- В бинарнике также есть шаблоны `Size of send data = {}`, `Size of recieve data = {}` и `Size of recieve sync data = {}`. В слове `recieve` сохранена опечатка из VCont.

Экспериментальная настройка только для текущего проекта/поставки `vcont-hsb`: для включения детального лога данных добавь в `vcontcfg.json`:

```json
"DataToLog": "serialize,deserialize"
```

В этой поставке после этого runtime пишет строки:

```text
SERIALIZE:имя_блока1:значение_DI_1;значение_DI_2/значение_DO_1;значение_DO_2||...
DESERIALIZE:имя_блока1:значение_DI_1;значение_DI_2/значение_DO_1;значение_DO_2||...
```

Пример:

```text
05-04-2026 11:48:03.511742841 [3639] elet.cpp:1095 LOG_DEBUG vcont SERIALIZE:FB_CTU_8:TRUE;FALSE;1000/FALSE;60||FB_CTU_2:TRUE;FALSE;1000/FALSE;60
```

Парсинг:

- направление `SERIALIZE` означает данные, которые узел сериализует для передачи;
- направление `DESERIALIZE` означает данные, которые узел применяет из полученного sync payload;
- блоки разделены `||`;
- внутри блока формат `имя:DI/DO`;
- значения DI и DO разделены `;`;
- при сравнении MAIN/RESERVE сопоставляй блоки по имени и учитывай порядок выполнения ФБ в bootfile.

До появления `DataToLog` наблюдение на программе из локальных ФБ (`BOOL/INT/NOT/AND/CTU/EQ`) было таким: в 5-минутном окне `Size of send data is 0` повторялось на обоих узлах, хотя пользовательская программа была загружена и исполнялась. Теперь для сценариев этого проекта при включенном `DataToLog` можно использовать `SERIALIZE/DESERIALIZE` как более точный диагностический oracle DI/DO, а `Size of send data` оставлять грубым признаком наличия payload. Для других VCont-проектов не предполагай наличие этих строк без отдельной проверки реализации.
...
- значения DI/DO в sync-потоке при включенном `DataToLog`: `SERIALIZE:...` и `DESERIALIZE:...`.
...
- Конкретные внутренние состояния ФБ, которые не попали в DI/DO сериализации.
...
Для проверки continuity данных после failover основной oracle все еще должен быть внешне наблюдаемым: Modbus server/alias, API/IDE-запросы, диагностические ФБ с выводом в доступную область или другой readback-механизм. `SERIALIZE/DESERIALIZE` в `vcont.log` полезен как дополнительный диагностический oracle HSB-синхронизации DI/DO и должен прикладываться к артефактам при расхождении MAIN/RESERVE.

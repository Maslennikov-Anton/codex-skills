...
- `AutoDBSaveInterval`: период автосохранения runtime DB/state в SQLite в секундах. `0` отключает автосохранение. Это не сохранение `.vcsys`/проекта VCStudio.
- `SaveOutputs`: сохранять ли данные выходных ножек функциональных блоков в runtime DB/state. `1` - сохранять, `0` - не сохранять.
- `LoadDBAtStartup`: загружает значения функциональных блоков из runtime DB/state при старте.
...
- `PlcId`: номер ПЛК в резервной паре/кластере. В актуальной HSB-election это `priority`: большее значение сильнее только после сравнения heartbeat `count`; при равных `count` и `PlcId` используется node `id`.
...
- `HsbSectionName`: имя секции параметров сетевого интерфейса в `ecal.ini`; значение по умолчанию - `hsb_network`.

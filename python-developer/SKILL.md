...
- Используй явную типизацию последовательно. Переменные, аргументы функций, возвращаемые значения и промежуточные коллекции должны быть типизированы везде, где это можно выразить разумно и без потери читаемости.
...
- При написании структурных комментариев используй русский язык; комментарии должны быть короткими, техническими и прямо описывать ответственность блока или модуля.
...
- Для SQL/data-access по умолчанию предпочитай `SQLAlchemy` + `Pydantic`, если в репозитории уже не закреплен другой стек, который нужно сохранять.
